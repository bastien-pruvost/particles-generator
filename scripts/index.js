const init = function () {
  ...
  const createBubble = function (size, opacity, color, speed, toTopPos, zIndex) {
    const bubble = document.createElement("span");
    const leftPos = getRandomBetween(-10, 105);
    const newSize = getRandomBetween((parseInt(size) - 15), (parseInt(size) + 15))
    ...
    bubble.style.setProperty("--leftPos", (leftPos + getRandomBetween(-10, 10)) + "%");

    bubble.style.setProperty("--animationTime", getRandomBetween(parseInt(speed - 500), parseInt(speed + 500)) + "ms");

    bubble.style.setProperty("--hueRotate", getRandomBetween(-13, 13) + "deg")
  ...
  // Layer 1
  setInterval(() => {
    for (let i = 0; i < (quantity1.value / 3); i++) {
      setTimeout(() => {
        createBubble(
          size1.value,
          opacity1.value,
  ...
  // Layer 2
  setInterval(() => {
    for (let i = 0; i < (quantity2.value / 3); i++) {
      setTimeout(() => {
        createBubble(
          size2.value,
          opacity2.value,
  ...
  // Layer 3
  setInterval(() => {
    for (let i = 0; i < (quantity3.value / 3); i++) {
      setTimeout(() => {
        createBubble(
          size3.value,
          opacity3.value,
  ...
  // Layer 4
  setInterval(() => {
    for (let i = 0; i < (quantity4.value / 3); i++) {
      setTimeout(() => {
        createBubble(
          size4.value,
          opacity4.value,
